...
Public Class frmAddEditStudentEvaluation
...
#Region "Database"
    ...
    Public Function GetEnrollmentDetails(ByVal strEDP As String) As Boolean
        ...
        Try
            ...
            If dtaTable.Tables(1).Rows.Count <= 0 Then
                Me.txtYear.Text = "0"
                Me.txtStatus.Text = ""
            ...
            'If Me.txtIsEnrolled.Text = 1 Then
            '    Me.NewEnrollment = False
            'Else
            '    Me.NewEnrollment = True
            'End If
    ...
    Private Function SaveStudentEnrollment(ByVal boolIsAdding As Boolean) As Boolean
        ...
        Dim strEnrolledStudentsSQL As String = "INSERT INTO enrolledstudents (" _
            & "StudentID,Term,SchoolYear,CourseCode,DeptCode,Adviser,Degree,EnrolStatus,EnrolYear) VALUES (" _
            & "?StudentID,?Term,?SchoolYear,?CourseCode,?DeptCode,?Adviser,?Degree,?EnrolStatus,?EnrolYear)"
        ...
        Dim strModifyEnrolledStudentsSQL As String = "DELETE FROM enrolledstudentssubjects WHERE " _
            ...
            & "UPDATE enrolledstudents SET CourseCode=?CourseCode," _
            & "DeptCode=?DeptCode,Degree=?Degree,EnrolStatus=?EnrolStatus,Term=?Term,SchoolYear=?SchoolYear,EnrolYear=?EnrolYear   " _
        ...
        Dim strEnrolledStudentsSubjectsSQL As String = "INSERT INTO enrolledstudentssubjects (" _
            & "SOID,StudentID,SubjectID,Term,SchoolYear,Remarks,SectionCode) VALUES (" _
            & "?SOID,?StudentID,?SubjectID,?Term,?SchoolYear,?Remarks,?SectionCode)"
        ...
        'command text for assessments details
        Dim strStudentChargesPerSemester As String = "" _
            & "INSERT INTO studentchargespersemester " _
            & "(EDPNumber,Term,SchoolYear,FeeID,FeeTitle,AmountToBePaid,Encoder,Remarks,FeeCat) VALUES " _
            & "(?EDPNumber,?Term,?SchoolYear,?FeeID,?FeeTitle,?AmountToBePaid,?Encoder,?Remarks,?FeeCat)"
            ...
            Using myconn As New MySqlConnection(ConnectionString)
                ...
                Try
                    ...
                    Using mycommanddetails As New MySqlCommand
                        ...
                        'add parameters
                        Dim SOID, StudentID, SubjectID, Term, SchoolYear, Remarks, SectionCode As MySqlParameter
...
#Region "Helper"
        ...
        Try
            ...
            For Each row As DataRow In TheTableOfFees.Rows
                If row("FeeCategory") = "Tuition Fee" Then
                    'assign tuition fee based on student's status (New,Old)
                    If Me.txtStatus.Text.ToUpper = "OLD" Then
                        If row("FeeName").ToString = "Tuition Fee Old" Then
                            decTuitionFee = row("Amount")
                        End If
                    Else
                        If row("FeeName").ToString = "Tuition Fee New" Then
                            decTuitionFee = row("Amount")
                        End If
                    End If
    ...
    Private Sub AssignFeesCalculatedIntoDataGridView()
    ...
    Private Sub ClearControls()
        ...
        With Me
            ...
            '.TheEDP = ""
            Me.txtIsEnrolled.Text = 0
            'Me.dvgSemestralFees.Rows.Clear()
        End With
    End Sub
...
#End Region
    ...
    Private Sub txtEDP_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtEDP.KeyPress
    ...
    Private Sub txtEDP_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtEDP.Validated
